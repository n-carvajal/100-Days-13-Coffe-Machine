...
def get_coins():
    ...
    Returns quarters, dimes, nickles, cents
    """
    while True:
        ...
        if not input_dimes.isdigit():
            ...
            continue
        input_nickles = input("Enter your nickles: ")
        if not input_nickles.isdigit():
            print("You did not enter a numerical value for nickles.")
            continue
        input_cents = input("Enter your cents: ")
        if not input_cents.isdigit():
            print("You did not enter a numerical value for cents.")
            continue
        return int(input_quarters), int(input_dimes), int(input_nickles), int(input_cents)


def coin_tally(entered_quarters, entered_dimes, entered_nickles, entered_cents):
    """
    Returns sum of inputted coins.
    """
    total_quarters = entered_quarters * coins["quarters"]
    total_dimes = entered_dimes * coins["dimes"]
    total_nickels = entered_nickles * coins["nickles"]
